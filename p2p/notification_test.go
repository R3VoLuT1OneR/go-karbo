...
func TestRawBlock_ToBlock20(t *testing.T) {
	...
	assert.Equal(t, config.TransactionVersion1, block.BaseTransaction.Version)
	assert.Equal(t, uint64(31), block.BaseTransaction.UnlockHeight)
	assert.Len(t, block.BaseTransaction.Inputs, 1)
	assert.Len(t, block.BaseTransaction.Outputs, 6)
	assert.Equal(t, []byte{0x1, 0x16, 0x8d, 0xbe, 0x0, 0x87, 0x71, 0x64, 0xa7, 0x33, 0xf, 0x18, 0x3c, 0x3d, 0xbf, 0x53, 0xce, 0x4, 0x21, 0xe8, 0x3a, 0xe9, 0x74, 0x76, 0xfd, 0x2, 0x55, 0xbc, 0x4a, 0x74, 0xcd, 0xe0, 0x59}, block.BaseTransaction.Extra)
}
